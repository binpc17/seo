...
if($lang>0){
 ...
 if($uDev>0){
	 $mob="Mob";
  if($lang!==''){
	 	$ps=$pdoJS->prepare("INSERT INTO webUsers (contryCD,countryN,twn,lng,app,track,track2,dateCreated) VALUES (213,'Algeria','JS',' $lang','$mob',1,'ZOLA',NOW())");
    $ps->execute();
...
<head>
  ...
  <title>Cherche travail? Ouedkniss emploi alger <?php echo date("Y"); ?> |eGoZola</title>
...
<strong style="color: black">Hey! Obtenez toutes les offres d'Emploi et d'Affaires en Algérie dans ton téléphone et bien plus encore, téléchargez notre application gratuite </strong> <strong style="color: darkgreen"> Cliquer ici↓↓</strong> <br>
	...
	<?php   
		...
		$urlWS=$botUrl;
$ps=$pdoJS->prepare("SELECT idJobS,sizeV,jobTitle,town,dateExp,urlJS,userN FROM jobservice WHERE idJobS>9 AND dateExp >= (select UNIX_TIMESTAMP()) ORDER BY idJobS DESC LIMIT 0,10");
$ps->execute();
...
?>
     ...
     <div style="text-align:center"> <h1> Cherche travail ou Ouedkniss emploi alger <?php echo date("Y"); ?>? </h1> </div> 
 <br> <span class="glyphicon glyphicon-tags"> </span>✓ Ouedkniss emploi alger <?php echo date("Y"); ?>: Trouve et Cherche travail ou une affaire rapidement en algérie, Oran, Constantine sur eGoZola
 <br> <span class="glyphicon glyphicon-tags"> </span>✓ Avec eGoZola, Retrouvez ou cherche travaile rapidement sur Ouedkniss emploi alger, Oran, Constantine et partout en Algérie  
 <br> <span class="glyphicon glyphicon-tags"> </span>✓ Publier ton annonce, vendez ton savoir et tes services et gagnez de l'argent. Retrouvez des partenaires d'affaire avec l'application eGoZola
  <br> <span class="glyphicon glyphicon-tags"> </span>✓ Retrouvez la personne, le travail ou l'affaire qu’il vous faut ou un service à domicile sur Ouedkniss emploi alger, blida et Oran. 
 <br> <span class="glyphicon glyphicon-tags"> </span>✓ Télecharger eGoZola et retrouvez des offres d'emploi dz <?php echo date("Y"); ?> sur Ouedkniss alger et d'affaires autour de moi et bien plus encore. <a href="<?php echo $botUrl?>" style="clolor:black"> Télécharger l'application <strong>Cliquer ici↓↓ </strong> </a>.
