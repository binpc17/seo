...
<head> 
 ...
 <title>Obtenez toutes les offres emploi jeunes ci  <?php echo date("Y"); ?> |eGoZola</title>
  <meta name="description" content="✓ Retrouvez et Postulez aux d’offres emploi jeunes ci <?php echo date("Y"); ?> complet, Téléchargez l'application emploi jeunes ci pour un accès sans internet et bien plus encore partout avec ton téléphone"/>
  <meta name="keywords" content="Cherche Emploi, emploi jeunes ci "/>
...
<div class="row" style="font-family: Calibri;margin-top: 50px">
 ...
 <div style="text-align:center;margin-top: 10px"> <h1>Obtenez toutes les offres emploi jeunes ci <?php echo date("Y"); ?></h1> </div> 
 <br> <span class="glyphicon glyphicon-tags"> </span>✓ emploi jeunes ci offre d'emploi: Trouve et Cherche du travaile ou un emploi rapidement en Cote d'ivoire sans vous déplacer
 <br> <span class="glyphicon glyphicon-tags"> </span>✓ Retrouvez rapidement une offre d’affaire ou emploi jeunes ci <?php echo date("Y"); ?> partout en Cote d'ivoire avec l'application <strong>eGoZola gratuite </strong>
 <br> <span class="glyphicon glyphicon-tags"> </span>✓ Avec eGoZola, Cherche travaile rapidement et retrouvez un travail sur emploi jeunes ci avec votre téléphone sans inernet et gratuit.
 <br> <span class="glyphicon glyphicon-tags"> </span>✓ Vendez ton savoir faire ou tes services partout au Burkina. Gagnez confidentiellement ton argent et développez rapidement ton réseau d’affaire et du Busness.
    <br> <span class="glyphicon glyphicon-tags"> </span>✓ <strong>Télecharger eGoZola et retrouvez toutes les offres emploi jeunes ci <?php echo date("Y"); ?></strong> et d'affaire autour de moi et bien plus encore. <a href="<?php echo $botUrl?>" style="clolor:black"> Télécharger l'application <strong style="color: darkgreen">Cliquer ici↓↓ </strong> </a>.
		<br>
		<br>
        ...
        <a href="<?php echo $botUrl?>" style="color: black">
         ...
         <strong style="color: black"> Pour Obtenir toutes les Offres d'emploi dans ton téléphone et d'affaires en Cote d'ivoire, téléchargez notre application gratuite </strong>  <strong style="color: darkgreen"> Cliquer ici ↓↓ </strong> <br>
			...
			<?php   
$ps=$pdoJS->prepare("SELECT idUser,idJobS,sizeV,jobTitle,town,dateExp,urlJS,userN FROM jobservice  WHERE countryCD=225 AND dateExp >= (select UNIX_TIMESTAMP()) ORDER BY idJobS DESC LIMIT 0,8");
$ps->execute();
$liste=$ps->fetchAll(PDO::FETCH_ASSOC); 
//echo ">>".sizeof($liste,0) >>> {$subjectData['urlJS']};
           echo "<ul class='list-group'>";
       foreach ($liste as $subjectData){
            echo "<li class='list-group-item'> <a href='".$botUrl."' target='_blank' style='color: black'> ".$subjectData['jobTitle']." <strong>".$subjectData['jobTitle']."</strong><br> <span class='glyphicon glyphicon-user'></span> ".$subjectData['userN']."| <em> Vieu(".$subjectData['sizeV'].") <span class='glyphicon glyphicon-map-marker'></span> ".$subjectData['town']."</em>";
